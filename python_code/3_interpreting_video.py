''' How to use these scripts: Please run these scripts in spyder, as they were made in spyder and I can't guarantee they will work from the command line, jupyter notebook etc. See the git repository readme for instructions on how to install the computing environment from the .yml file
    ...
    This script is very similar to 1_extracting_from_video.py, the difference is that:
    (1) Instead of just pulling out the numbers, it pulls out the numbers and then passes the number to the machine learning model
    (2) we are pointing it at a different video file to see how it performs on new footage
    Outputs are saved in exampleML\outputs\output_data_interpretations. 
    The machine learning interpretations are printed in the top left of the images loose in this folder
    The interpretations are also saved numerically in detections[your_date_here].csv
    This 'detections' csv file is the reason we did all this. 
    In theory, the camera can continue to read the numbers off the display to collect the fish weight data being generated by this fish sorting machine (grading machine)
    In reality, our machine learning model should be trained on more varied data, and much more of it, to perform well in a real environment
'''
...
# Load ML model
model=load_model(package_directory+r'\outputs\trained_model\digits_trained_model_JR.h5')		#import CNN model weight

# Trig approximation for tansformation. This is purely empirical. I tried to use cameratransform package but it wasn't working and documentation explains it poorly
...
outputs_directory=package_directory+'/outputs/output_data_interpretations/'
...
wkingdir=package_directory+r'\outputs\output_data_interpretations\\'
os.chdir(wkingdir)
                ...
                if diffcurrent<diffprevious: # Take a snap once the frames have stabilised. For a rolling video there is a 'peak' in the difference about 2 frames after the number starts to change 
                    ...
                    for c in cnts:
                        ...
                        if cv2.contourArea(c) > 1000:
                            ...
                            im_final = im_bw.reshape(1,28,28,1)# required shape
                            ans = model.predict(im_final)
                            # ans is array of possibility of respective digit. choose the digit with greatest possibility as predicted dight
                            maxval=max(ans[0].tolist())
                            ans = ans[0].tolist().index(maxval)
                            if ans == 10:
                                ans = 'g'
                            digits.append(str(ans))
                            
                            # Check result and write it on the image file 
                            if save_images==True:
                                cv2.imwrite(package_directory+'/outputs/output_data_interpretations/'+str(ans)+"/motion_detected_at_"+str(framedtime)+str(i_n)+".png",im_bw)                            
                                if print_result:
                                    img = Image.open(package_directory+'/outputs/output_data_interpretations/'+str(ans)+"/motion_detected_at_"+str(framedtime)+str(i_n)+".png")
                                    draw = ImageDraw.Draw(img)
                                    font = ImageFont.truetype('arial',24)
                                    draw.text((0, 0),str(ans),font=font, fill=(155))
                                    img.save(package_directory+'/outputs/output_data_interpretations/'+str(ans)+"/motion_detected_at_"+str(framedtime)+str(i_n)+".png")

                    digits = ''.join(digits)
                    digitlist.append(str(digits))
                    timeslist.append(str(framedtime))
                    
                    if len(digitlist)==1000:
                        recorded_data = pd.DataFrame(digitlist,timeslist,columns=['interpreted_text'])
                        recorded_data.to_csv("detections_"+str(framedtime)+".csv")
                        digitlist = []
                        timeslist = []

                    if save_images==True:
                        cv2.imwrite("motion_detected_at_"+str(framedtime)+".png", thresh)
                        if print_result:
                            img = Image.open("motion_detected_at_"+str(framedtime)+".png")
                            draw = ImageDraw.Draw(img)
                            font = ImageFont.truetype('arial',24)
                            draw.text((0, 0),str(digits),font=font, fill=(155))
                            img.save("motion_detected_at_"+str(framedtime)+".png")
                    (digits,time)
...
recorded_data = pd.DataFrame(digitlist,timeslist,columns=['interpreted_text'])
recorded_data.to_csv("detections_"+str(datetime.datetime.today().strftime('%Y_%m_%d_%H_%M_%S_%f'))+".csv")
